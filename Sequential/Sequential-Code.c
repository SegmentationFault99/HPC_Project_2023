...
void write_to_file(int width, int heigth, double total_time, int what_file)
{
	FILE * f;

	if (what_file == 0)
		f = fopen("Sequential-Results.csv", "a");
	else
		f = fopen("Sequential-Vectorized-Results.csv", "a");
...
void play(int width, int heigth, int file_flag)
{
	int i, j, k;
	...
	write_to_file(width, heigth, total_time, file_flag);
...
{
	//width, heigth and file flag are initialized with their default values
	int width = DEFAULT_WIDTH,
		heigth = DEFAULT_HEIGTH,
		file_flag = DEFAULT_FILE;
	...
	if (atoi(argv[3]) > DEFAULT_FILE)
		file_flag = atoi(argv[3]);

	play(width, heigth, file_flag);
}